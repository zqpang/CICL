...
def compute_jaccard_distance(target_features, k1=20, k2=6, print_flag=True, search_option=0, use_float16=False):
    ...
    if (search_option==0):
        ...
        _, initial_rank = search_raw_array_pytorch(res, target_features, target_features, k1)#通过一种方法直接搜索最近邻？
    ...
    for i in range(N):
        ...
        dist = 2-2*torch.mm(target_features[i].unsqueeze(0).contiguous(), target_features[k_reciprocal_expansion_index].t())#计算距离
        if use_float16:
    ...
    if k2 != 1:
        ...
        for i in range(N):
            V_qe[i,:] = np.mean(V[initial_rank[i,:k2],:], axis=0)#近邻重新做均值
        V = V_qe
        del V_qe
    ...
    #print("jaccard_dist: ", jaccard_dist)
    #print("len(jaccard_dist): ", len(jaccard_dist))

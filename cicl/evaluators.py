...
def extract_features(model, data_loader, print_freq=50):
    ...
    with torch.no_grad():
        for i, (imgs, imgs2, imgs3, fnames, pids,  clothesid, _,  _)  in enumerate(data_loader):
            
            data_time.update(time.time() - end)
            outputs = extract_cnn_feature(model, imgs)
            outputs_2 = extract_cnn_feature(model,imgs2)
            outputs_3 = extract_cnn_feature(model,imgs3)
            for fname, output, output_2, output_3, pid in zip(fnames, outputs, outputs_2, outputs_3, pids):
                features[fname] = output
                features2[fname] = output_2
                features3[fname] = output_3
                labels[fname] = pid
    ...
    return features, features2, features3, labels
...
def evaluate_all(query_features, gallery_features, distmat, query=None, gallery=None,
    ...
    # Compute mean AP
    mAP = mean_ap(distmat, query_ids, gallery_ids, query_cams, gallery_cams, query_clothes, gallery_clothes)
...
#         return mAP
    ...
    cmc_scores = {name: cmc(distmat, query_ids, gallery_ids,
                            query_cams, gallery_cams, query_clothes, gallery_clothes, **params)
...
class Evaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery, cmc_flag=False, rerank=False):
        features, _ , _, _ = extract_features(self.model, data_loader)
